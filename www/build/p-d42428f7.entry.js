...
import { a as n, g as a, w as o } from './p-67dfcaed.js';
const h = new Map(),
  r = new DOMParser(),
  c = class {
    constructor(i) {
      t(this, i),
        (this.slLoad = s(this, 'sl-load', 7)),
        (this.slError = s(this, 'sl-error', 7)),
    ...
    getLabel() {
      ...
      return (
        this.label
          ...
          : this.src && (t = this.src.replace(/.*\//, '').replace(/-/g, ' ').replace(/\.svg/i, '')),
        t
      );
    }
    async setIcon() {
      const t = a(this.library);
      let s = this.src;
      if ((this.name && t && (s = t.resolver(this.name)), s))
        try {
          const i = await (t => {
            if (h.has(t)) return h.get(t);
            {
              const s = fetch(t).then(async t => {
                if (t.ok) {
          ...
          if (i.ok) {
            const s = r.parseFromString(i.svg, 'text/html').body.querySelector('svg');
            s
              ? (t && t.mutator && t.mutator(s), (this.svg = s.outerHTML), this.slLoad.emit())
              : ((this.svg = ''), this.slError.emit({ status: i.status }));
          }
        } catch (t) {
          this.slError.emit();
        }
      else this.svg && (this.svg = null);
    }
    render() {
      return i('div', { part: 'base', class: 'icon', role: 'img', 'aria-label': this.getLabel(), innerHTML: this.svg });
    }
