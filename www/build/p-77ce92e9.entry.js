import { r, c as o, h as t } from './p-32052503.js';
const i = class {
  constructor(t) {
    r(this, t),
      ...
      (this.step = 1),
      (this.tooltip = 'top'),
      (this.tooltipFormatter = r => r.toString());
  }
  connectedCallback() {
  ...
  componentWillLoad() {
    null == this.value && (this.value = this.min),
      this.value < this.min && (this.value = this.min),
      this.value > this.max && (this.value = this.max);
  }
  componentDidLoad() {
    this.syncTooltip(), (this.resizeObserver = new ResizeObserver(() => this.syncTooltip()));
  }
  async setFocus(r) {
    this.input.focus(r);
  }
  ...
  async setCustomValidity(r) {
    this.input.setCustomValidity(r), (this.invalid = !this.input.checkValidity());
  }
  handleInput() {
    (this.value = Number(this.input.value)), this.slChange.emit(), requestAnimationFrame(() => this.syncTooltip());
  }
  handleBlur() {
    (this.hasFocus = !1), (this.hasTooltip = !1), this.slBlur.emit(), this.resizeObserver.unobserve(this.input);
  }
  handleFocus() {
    (this.hasFocus = !0), (this.hasTooltip = !0), this.slFocus.emit(), this.resizeObserver.observe(this.input);
  }
  handleTouchStart() {
    this.setFocus();
  }
  syncTooltip() {
    if ('none' !== this.tooltip) {
      const r = Math.max(0, (this.value - this.min) / (this.max - this.min)),
        o = this.input.offsetWidth,
        t = this.output.offsetWidth,
        i = getComputedStyle(this.input).getPropertyValue('--thumb-size');
      (this.output.style.transform = `translateX(calc(${o * r}px - calc(calc(${r} * ${i}) - calc(${i} / 2))))`),
        (this.output.style.marginLeft = `-${t / 2}px`);
    }
  }
  render() {
    return t(
      'div',
      {
        ...
        class: {
          ...
          'range--tooltip-top': 'top' === this.tooltip,
          'range--tooltip-bottom': 'bottom' === this.tooltip
        },
      ...
      'none' !== this.tooltip &&
        t(
          'output',
          { part: 'tooltip', ref: r => (this.output = r), class: 'range__tooltip' },
