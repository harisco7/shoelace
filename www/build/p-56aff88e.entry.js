import { r as t, c as e, h as s, g as l } from './p-32052503.js';
import { g as i, h as a } from './p-e1b02254.js';
import { F as o } from './p-4608aa48.js';
let r = 0;
const n = class {
  constructor(s) {
    t(this, s),
      ...
      (this.inputId = 'select-' + ++r),
      (this.labelId = 'select-label-' + r),
      (this.helpTextId = 'select-help-text-' + r),
      (this.hasFocus = !1),
      (this.hasHelpTextSlot = !1),
      (this.hasLabelSlot = !1),
      (this.isOpen = !1),
      (this.items = []),
      (this.displayLabel = ''),
  ...
  handleMultipleChange() {
    const t = this.getValueAsArray();
    (this.value = this.multiple ? t : t[0] || ''), this.syncItemsFromValue();
  }
  handleValueChange() {
    this.syncItemsFromValue(), this.slChange.emit();
  }
  connectedCallback() {
  ...
  getItemLabel(t) {
    const e = t.shadowRoot.querySelector('slot:not([name])');
    return i(e);
  }
  getItems() {
  ...
  handleClearClick(t) {
    t.stopPropagation(), (this.value = this.multiple ? [] : ''), this.syncItemsFromValue();
  }
  handleKeyDown(t) {
    const e = t.target,
      s = this.getItems(),
    ...
    if ('sl-tag' !== e.tagName.toLowerCase())
      if ('Tab' !== t.key) {
        if (['ArrowDown', 'ArrowUp'].includes(t.key)) {
          if ((t.preventDefault(), this.isOpen || this.dropdown.show(), 'ArrowDown' === t.key && l))
            return void l.setFocus();
          if ('ArrowUp' === t.key && i) return void i.setFocus();
        }
        this.isOpen || (t.stopPropagation(), t.preventDefault(), this.dropdown.show(), this.menu.typeToSelect(t.key));
  ...
  handleMenuShow(t) {
    this.disabled
      ? t.preventDefault()
      : (this.resizeMenu(), this.resizeObserver.observe(this.host), (this.isOpen = !0));
  }
  handleMenuHide() {
    this.resizeObserver.unobserve(this.host), (this.isOpen = !1);
  }
  handleSlotChange() {
    (this.hasHelpTextSlot = a(this.host, 'help-text')),
      (this.hasLabelSlot = a(this.host, 'label')),
      this.syncItemsFromValue(),
      this.reportDuplicateItemValues();
  }
  handleTagInteraction(t) {
    t.composedPath().find(t => {
      if (t instanceof HTMLElement) return 'clear-button' === t.getAttribute('part');
    }) && t.stopPropagation();
  }
  reportDuplicateItemValues() {
    ...
    if (t.length) throw new Error('Duplicate value found on <sl-menu-item> in <sl-select>: "' + t.join('", "') + '"');
  }
  resizeMenu() {
  ...
  syncItemsFromValue() {
    ...
    if ((t.map(t => (t.checked = e.includes(t.value))), this.multiple)) {
      const l = [];
      if (
        (e.map(e => t.map(t => (t.value === e ? l.push(t) : null))),
        (this.displayTags = l.map(t =>
          s(
            'sl-tag',
            {
              ...
              'onSl-clear': e => {
                e.stopPropagation(), this.disabled || ((t.checked = !1), this.syncValueFromItems());
              }
            },
            this.getItemLabel(t)
          )
        )),
        this.maxTagsVisible > 0 && this.displayTags.length > this.maxTagsVisible)
      ) {
        const t = this.displayTags.length;
        (this.displayLabel = ''),
          (this.displayTags = this.displayTags.slice(0, this.maxTagsVisible)),
          this.displayTags.push(
            s('sl-tag', { exportparts: 'base:tag', type: 'info', size: this.size }, '+', t - this.maxTagsVisible)
          );
      }
    } else {
      const s = t.filter(t => t.value === e[0])[0];
      (this.displayLabel = s ? this.getItemLabel(s) : ''), (this.displayTags = []);
    }
  }
  syncValueFromItems() {
    const t = this.getItems()
      .filter(t => t.checked)
      .map(t => t.value);
    this.value = this.multiple ? this.value.filter(e => t.includes(e)) : t.length > 0 ? t[0] : '';
  }
  render() {
    var t;
    const e = this.multiple ? this.value.length > 0 : '' !== this.value;
    return s(
      o,
      {
      ...
      s(
        'sl-dropdown',
        {
          ...
          class: {
            ...
            'select--empty': 0 === (null === (t = this.value) || void 0 === t ? void 0 : t.length),
            'select--focused': this.hasFocus,
            'select--clearable': this.clearable,
            'select--disabled': this.disabled,
            'select--multiple': this.multiple,
            'select--has-tags': this.multiple && e,
        ...
        s(
          'sl-menu',
          { ref: t => (this.menu = t), part: 'menu', class: 'select__menu', 'onSl-select': this.handleMenuSelect },
