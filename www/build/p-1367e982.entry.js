import { r as t, c as o, h as r, g as s } from './p-32052503.js';
import { f as i } from './p-64fa07d7.js';
import { s as a, g as e } from './p-b8363807.js';
const l = class {
  constructor(r) {
    t(this, r),
  ...
  componentDidLoad() {
    new IntersectionObserver((t, o) => {
      ...
      (this.mutationObserver = new MutationObserver(t => {
        t.some(t => !['aria-labelledby', 'aria-controls'].includes(t.attributeName)) &&
          setTimeout(() => this.setAriaLabels());
      })),
      this.mutationObserver.observe(this.host, { attributes: !0, childList: !0, subtree: !0 });
  }
  disconnectedCallback() {
    this.mutationObserver.disconnect(), i.unobserve(this.tabGroup), this.resizeObserver.unobserve(this.nav);
  }
  async show(t) {
    const o = this.getAllTabs().find(o => o.panel === t);
    o && this.setActiveTab(o);
  }
  getAllTabs(t = !1) {
    return [...this.tabs.querySelector('slot').assignedElements()].filter(o =>
      t ? 'sl-tab' === o.tagName.toLowerCase() : 'sl-tab' === o.tagName.toLowerCase() && !o.disabled
    );
  }
  getAllPanels() {
    return [...this.body.querySelector('slot').assignedElements()].filter(
      t => 'sl-tab-panel' === t.tagName.toLowerCase()
    );
  }
  getActiveTab() {
    return this.getAllTabs().find(t => t.active);
  }
  handleClick(t) {
    const o = t.target.closest('sl-tab');
    if (o.closest('sl-tab-group') !== this.host) return !1;
    o && this.setActiveTab(o);
  }
  handleKeyDown(t) {
    const o = t.target.closest('sl-tab');
    if (o.closest('sl-tab-group') !== this.host) return !1;
    if (
      (['Enter', ' '].includes(t.key) && o && (this.setActiveTab(o), t.preventDefault()),
      ['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown', 'Home', 'End'].includes(t.key))
    ) {
      const o = document.activeElement;
      if (o && 'sl-tab' === o.tagName.toLowerCase()) {
        const r = this.getAllTabs();
        let s = r.indexOf(o);
        'Home' === t.key
          ? (s = 0)
          : 'End' === t.key
          ...
          : 'ArrowLeft' === t.key
          ? (s = Math.max(0, s - 1))
          : 'ArrowRight' === t.key && (s = Math.min(r.length - 1, s + 1)),
          r[s].setFocus({ preventScroll: !0 }),
          ['top', 'bottom'].includes(this.placement) && a(r[s], this.nav, 'horizontal'),
          t.preventDefault();
      }
    }
  }
  ...
  setActiveTab(t, o = !0) {
    if (t && t !== this.activeTab && !t.disabled) {
      ...
      (this.activeTab = t),
        this.getAllTabs().map(t => (t.active = t === this.activeTab)),
        this.getAllPanels().map(t => (t.active = t.name === this.activeTab.panel)),
        this.syncActiveTabIndicator(),
        ['top', 'bottom'].includes(this.placement) && a(this.activeTab, this.nav, 'horizontal'),
        o && (r && this.slTabHide.emit({ name: r.panel }), this.slTabShow.emit({ name: this.activeTab.panel }));
    }
  }
  setAriaLabels() {
    const t = this.getAllTabs(),
      o = this.getAllPanels();
    t.map(t => {
      const r = o.find(o => o.name === t.panel);
      r &&
  ...
  syncActiveTabIndicator() {
    const t = this.getActiveTab(),
      ...
      r = t.clientHeight,
      s = e(t, this.nav),
      i = s.top + this.nav.scrollTop,
      a = s.left + this.nav.scrollLeft;
    switch (this.placement) {
      case 'top':
      case 'bottom':
        (this.activeTabIndicator.style.width = o + 'px'),
          (this.activeTabIndicator.style.height = null),
          (this.activeTabIndicator.style.transform = `translateX(${a}px)`);
        break;
      case 'left':
      case 'right':
        (this.activeTabIndicator.style.width = null),
          (this.activeTabIndicator.style.height = r + 'px'),
          (this.activeTabIndicator.style.transform = `translateY(${i}px)`);
    }
  }
  render() {
    return r(
      ...
      {
        part: 'base',
        ref: t => (this.tabGroup = t),
        class: {
          'tab-group': !0,
          'tab-group--top': 'top' === this.placement,
          'tab-group--bottom': 'bottom' === this.placement,
          'tab-group--left': 'left' === this.placement,
          'tab-group--right': 'right' === this.placement,
      ...
      r(
        ...
        r(
          'div',
          { ref: t => (this.nav = t), key: 'nav', part: 'nav', class: 'tab-group__nav', tabindex: '-1' },
          r(
            'div',
            { ref: t => (this.tabs = t), part: 'tabs', class: 'tab-group__tabs', role: 'tablist' },
            r('div', {
              ref: t => (this.activeTabIndicator = t),
      ...
      r('div', { ref: t => (this.body = t), part: 'body', class: 'tab-group__body' }, r('slot', null))
    );
  }
  get host() {
    return s(this);
